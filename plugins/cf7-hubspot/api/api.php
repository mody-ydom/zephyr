<?php
		...
		class vxcf_hubspot_api extends vxcf_hubspot{
			...
			public function post_hubspot_arr( $path, $method = 'get', $body = "" ){
				...
				if( isset( $hubspot_response['status'] ) && $hubspot_response['status'] == 'error' && ! empty( $hubspot_response['message'] ) &&
				    strpos( $hubspot_response['message'], 'expired' ) !== false ){
					$get_token = true;
				}
			...
			public function get_crm_fields( $object ){
				...
				if( strpos( $object, 'vxf_' ) !== false ){
					...
					if( ! empty( $res['formFieldGroups'] ) ){
						...
						$fields['skipValidation'] = array(
							'name'    => 'skipValidation',
							'label'   => 'Skip Validation',
							'type'    => 'boolean',
							'eg'      => '0 or 1',
							'options' => array( array( 'label' => 'True', 'value' => '1' ), array( 'label' => 'False', 'value' => '0' ) )
						);
					} else{
				...
				if( $object == 'Task' ){
					return array(
						'name'        => array( 'name' => 'name', 'label' => 'Title', 'type' => 'Text' ),
						'description' => array( 'name' => 'description', 'label' => 'Note', 'type' => 'Text', 'req' => 'true' ),
						'timestamp'   => array( 'name' => 'timestamp', 'label' => 'Due Date', 'type' => 'Datetime' )
					);
				}
				$module = 'contacts';
				$v      = 'v1';
			...
			public function push_object( $object, $fields, $meta ){
				...
				$product_post = array(
					array( 'name' => 'name', 'value' => 'Firstxx Product from API' ),
					array( 'name' => 'price', 'value' => 270 ),
					array( 'name' => 'description', 'value' => '<b>product description is here</b>' )
				);
				
				$item_post = array(
					array( 'name' => 'name', 'value' => 'First Product from API' ),
					array( 'name' => 'price', 'value' => 200 ),
					array( 'name' => 'quantity', 'value' => 20 ),
					array( 'name' => 'hs_product_id', 'value' => '28874673' )
				);
				
				$deal = array(
					array( 'externalPropertyName' => 'name', 'hubspotPropertyName' => 'dealname', 'dataType' => 'STRING' ),
					array( 'externalPropertyName' => 'price', 'hubspotPropertyName' => 'amount', 'dataType' => 'STRING' ),
					array( 'externalPropertyName' => 'dealstage', 'hubspotPropertyName' => 'dealstage', 'dataType' => 'STRING' )
				);
				
				$product = array(
					array( 'externalPropertyName' => 'name', 'hubspotPropertyName' => 'name', 'dataType' => 'STRING' ),
					array( 'externalPropertyName' => 'price', 'hubspotPropertyName' => 'price', 'dataType' => 'STRING' ),
					array( 'externalPropertyName' => 'description', 'hubspotPropertyName' => 'description', 'dataType' => 'STRING' )
				);
				$json    = '[
        {
        ...
        },
        {
          ...
          "dataType": "STRING"
				...
				$post    = array(
					'enabled'  => true,
					'mappings' => array(
						'DEAL'      => array( 'properties' => $deal ),
						'CONTACT'   => array( 'properties' => $con ),
						'PRODUCT'   => array( 'properties' => $product ),
						'LINE_ITEM' => array( 'properties' => $item )
					)
				);
  ...
  <p><?php print_r( $hubspot_response ) ?></p>
				...
				$path      = '';
				$send_body = false;
				$post      = array(); //$meta['_vx_contact_id']=array('value' =>301);
				//if($error ==""){
				if( $id == "" ){
					$action    = "Added";
					$send_body = true;
...
//'communications'=>array(array('value'=>true,'text'=>'Text','subscriptionTypeId'=>'5388809'))
						...
						if( ! empty( $this->info['portal_id'] ) && ! empty( $form_id ) && ! empty( $fields ) ){
							...
							if( ! isset( $fields['vx_title'] ) ){
								$title = '';
								if( ! empty( $meta['_vx_entry']['_vx_title'] ) ){
									$title = $meta['_vx_entry']['_vx_title'];
								} else if( ! empty( $meta['_vx_entry']['_vx_form_name'] ) ){
									$title = $meta['_vx_entry']['_vx_form_name'];
								}
								if( ! empty( $title ) ){
							...
							foreach( $fields as $k => $v ){
								if( isset( $context[ $k ] ) ){
									...
									$post['legalConsentOptions'] = array(
										'consent' => array(
									...
									if( ! empty( $item['p_id'] ) ){
										...
										if( empty( $hub_id ) ){
											$product_post            = array(
												...
												array( 'name' => 'price', 'value' => $item['unit_price'] ),
												array( 'name' => 'description', 'value' => esc_html( $item['desc'] ) )
											);
											$path                    = 'crm-objects/v1/objects/products';
										...
										$item_post            = array(
											array( 'name' => 'name', 'value' => $item['title'] ),
											array( 'name' => 'price', 'value' => $item['unit_price'] ),
											array( 'name' => 'quantity', 'value' => $item['qty'] ),
											array( 'name' => 'hs_product_id', 'value' => $hub_id )
										);
										$path                 = 'crm-objects/v1/objects/line_items';
			...
			public function get_wc_items( $meta ){
				...
				if( is_array( $items ) && count( $items ) > 0 ){
					foreach( $items as $item_id => $item ){
						...
						$sku = $desc = '';
						$qty = $unit_price = $tax = $total = $p_id = 0;
						if( method_exists( $item, 'get_product' ) ){
							// $p_id=$v->get_product_id();
							$product = $item->get_product();
							if( ! $product ){
								continue;
							} //product deleted but exists in line items of old order
