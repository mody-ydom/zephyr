<?php
  ...
  limit $start , $per_page";
			...
			public function get_object_feeds( $form_id, $account, $object, $skip = '' ){
				...
				$table_name = $this->get_crm_table_name();
				$sql        = $wpdb->prepare(
					"SELECT * FROM $table_name WHERE id!=%d and form_id=%s and object = %s and account=%d and is_active=1 ORDER BY sort",
					$skip,
					$form_id,
					$object,
					$account );
			...
			public function get_feed_log( $feed_id, $order_id, $object, $parent_id = "" ){
				...
				$table   = $this->get_crm_table_name( 'log' );
				$sql     = $wpdb->prepare(
					'SELECT * FROM ' . $table . ' where entry_id = %s and feed_id = %d and crm_id!="" and object=%s  and parent_id=%d order by id desc limit 1',
					$order_id,
					$feed_id,
					$object,
					$parent_id );
