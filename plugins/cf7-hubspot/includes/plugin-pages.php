		...
		class vxcf_hubspot_pages extends vxcf_hubspot{
			...
			public function setup_plugin(){
				...
				if( $this->post( 'vx_tab_action_' . $this->id ) == "export_log" ){
					...
					$field_titles = array(
						"#",
						__( 'Status', 'contact-form-hubspot-crm' ),
						__( 'HubSpot ID', 'contact-form-hubspot-crm' ),
						__( 'Entry ID', 'contact-form-hubspot-crm' ),
						__( 'Description', 'contact-form-hubspot-crm' ),
						__( 'Time', 'contact-form-hubspot-crm' )
					);
				...
				self::$tooltips = array(
					...
					'vx_sel_form'    => __(
						'Select the Contact Form you would like to integrate with HubSpot. Contacts generated by this form will be automatically added to your HubSpot account.',
					...
					'vx_optin_condition' => __(
						'When the opt-in condition is enabled, form submissions will only be exported to HubSpot when the condition is met. When disabled all form submissions will be exported.',
						'contact-form-hubspot-crm' ),
					
					'vx_manual_export' => __(
						'If you do not want all entries sent to HubSpot, but only specific, approved entries, check this box. To manually send an entry to HubSpot, go to Entries, choose the entry you would like to send to HubSpot, and then click the "Send to HubSpot" button.',
						'contact-form-hubspot-crm' ),
					
					'vx_entry_notes' => __(
						'Enable this option if you want to synchronize Contact Form entry notes to HubSpot Object notes. For example , when you add a note to a Contact Form entry, it will be added to the HubSpot Object selected in the feed.',
						'contact-form-hubspot-crm' ),
					...
					'vx_disable_logs' => __(
						'When an order is sent to HubSpot we store that order information in the database and show it in the HubSpot Log. Check this box if you do not want to save the exported order information in the logs.',
						'contact-form-hubspot-crm' ),
					...
					'vx_order_notes' => __(
						'Enable this option if you want to synchronize WooCommerce Order notes to HubSpot Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the HubSpot Object selected in the feed.',
						'contact-form-hubspot-crm' ),
			...
			public function format_log_msg( $log ){
				...
				$icons = array(
					'0' => array( 'color' => '#DC513B', 'icon' => 'fa-warning' ),
					'4' => array( 'color' => '#3897C3', 'icon' => 'fa-filter' ),
					'2' => array( 'color' => '#d5962c', 'icon' => 'fa-edit' ),
					'5' => array( 'color' => '#DC513B', 'icon' => 'fa-times' )
				);
			...
			public function mapping_page(){
				...
				$tabs = array(
					'feeds'    => __( 'HubSpot Feeds', 'contact-form-hubspot-crm' ),
					'logs'     => __( 'HubSpot Log', 'contact-form-hubspot-crm' ),
					'accounts' => __( 'HubSpot Accounts', 'contact-form-hubspot-crm' ),
					'settings' => __( 'Settings', 'contact-form-hubspot-crm' )
				);
                ...
                <h2 class="nav-tab-wrapper">
					<?php
						...
						foreach( $tabs as $k => $v ){
							...
							?>
                          <a href="<?php echo $link . '&tab=' . $k ?>" class="nav-tab <?php if( $k == $tab ){
							  echo 'nav-tab-active';
						  } ?>"><?php echo $v; ?></a>
			...
			public function log_page(){
				...
				$data       = $this->data->get_log();
				$items      = count( $data['feeds'] );
				...
				if( isset( $_REQUEST['orderby'] ) ){
					switch( $_REQUEST['orderby'] ){
						case"crm_id":
							$crm_order = $order_icon;
							$crm_class = "";
							break;
						case"entry_id":
							$entry_order = $order_icon;
							$entry_class = "";
							break;
						case"object":
							$desc_order = $order_icon;
							$desc_class = "";
							break;
						case"time":
				...
				$bulk_actions = array(
					""                       => __( 'Bulk Action', 'contact-form-hubspot-crm' ),
					"delete"                 => __( 'Delete', 'contact-form-hubspot-crm' ),
					'send_to_crm_bulk'       => __( 'Send to HubSpot', 'contact-form-hubspot-crm' ),
					'send_to_crm_bulk_force' => __( 'Force Send to HubSpot - Ignore filters', 'contact-form-hubspot-crm' )
				);
				$base_url     = $this->get_base_url();
				$objects      = $this->get_objects();
				$statuses     = array(
					"1"     => __( "Created", 'contact-form-hubspot-crm' ),
					"2"     => __( "Updated", 'contact-form-hubspot-crm' ),
					"error" => __( "Failed", 'contact-form-hubspot-crm' ),
					"4"     => __( "Filtered", 'contact-form-hubspot-crm' ),
					"5"     => __( "Deleted", 'contact-form-hubspot-crm' )
				);
   ...
   $refresh=true;
			...
			public function get_filter_ops(){
				return array(
					...
					"is_in"        => "(Text) Is In",
					"not_in"       => "(Text) Is Not In",
					"starts"       => "(Text) Starts With",
					"not_starts"   => "(Text) Does Not Start With",
					"ends"         => "(Text) Ends With",
					"not_ends"     => "(Text) Does Not End With",
					"less"         => "(Number) Less Than",
					"greater"      => "(Number) Greater Than",
					"less_date"    => "(Date/Time) Less Than",
					"greater_date" => "(Date/Time) Greater Than",
					"equal_date"   => "(Date/Time) Equals",
					"empty"        => "Is Empty",
			...
			public function log_detail(){
				...
				$triggers = array(
					...
					'add_note'    => 'Entry Note Created',
					'delete_note' => 'Entry Note Deleted'
				);
			...
			public function settings_page(){
				...
				if( ! empty( $_POST[ $this->id . "_uninstall" ] ) ){
					...
					$uninstall_msg = sprintf(
						__( "Contact Form HubSpot Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-hubspot-crm' ),
						"<a href='plugins.php'>",
						"</a>" );
