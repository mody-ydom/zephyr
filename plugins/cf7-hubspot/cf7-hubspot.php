...
die();*/
			...
			public function display_msg( $type, $message, $id = "" ){
				...
				if( $type == "admin" ){
					if( $ver < 4.2 ){
						?>
                      <div class="error vx_notice notice" data-id="<?php echo $id ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span
                              class="dashicons dashicons-megaphone"></span> <b><?php _e( 'Contact Form HubSpot Plugin', 'contact-form-hubspot-crm' ) ?>. </b><?php echo wp_kses_post(
									  $message ); ?> </span>
                          <span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss"
                                                                                                            title="<?php _e( 'Dismiss Notice', 'contact-form-hubspot-crm' ) ?>">dismiss</a></span></p>
                      </div>
						<?php
					} else{
						?>
                      <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e(
									  'Contact Form HubSpot Plugin',
									  'contact-form-hubspot-crm' ) ?>. </b> <?php echo wp_kses_post( $message ); ?> </p>
                      </div>
						<?php
					}
				} else{
                  ...
                  <tr class="plugin-update-tr">
                    <td colspan="5" class="plugin-update">
                      ...
                      <div style="background-color: rgba(224, 224, 224, 0.5);  padding: 9px; margin: 0px 10px 10px 28px ">
                        <div style="background-color: #d54d21; padding: 5px 10px; color: #fff" class="vx_msg"><span class="dashicons dashicons-info"></span> <?php echo wp_kses_post( $message ) ?>
                        </div>
                      </div>
                    </td>
                  </tr>
			...
			public function cf_status(){
				...
				if( $installed != 1 ){
					if( $installed === 0 ){ // not found
						$message = sprintf(
							__( "%sContact Form Entries%s is required. %s it today!%s", 'contact-form-hubspot-crm' ),
							"<a href='https://www.crmperks.com/'>",
							"</a>",
							"<a href='https://www.crmperks.com/'>",
							"</a>" );
					} else if( $installed === 2 ){ // not active
						$message = sprintf(
							__( 'Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form HubSpot Plugin', 'contact-form-hubspot-crm' ),
							'<strong><a href="' . wp_nonce_url(
								admin_url( 'plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php' ),
								'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php' ) . '">',
							'</a></strong>' );
					} else if( $installed === 3 ){ // not supported
						$message = sprintf(
							__( "A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'contact-form-hubspot-crm' ),
							"<a href='https://www.crmperks.com/'>",
							"</a>",
							"<a href='https://www.crmperks.com/'>",
							"</a>" );
							...
							foreach( $filter_s as $filter ){
								...
								switch( $filter['op'] ){
									...
									case"is_not":
										...
										break;
									case"contains":
										$check = strpos( $val, $fval ) !== false;
										break;
									case"not_contains":
										$check = strpos( $val, $fval ) === false;
										break;
									case"is_in":
										$check = strpos( $fval, $val ) !== false;
										break;
									case"not_in":
										$check = strpos( $fval, $val ) === false;
										break;
									case"starts":
										$check = strpos( $val, $fval ) === 0;
										break;
									case"not_starts":
										$check = strpos( $val, $fval ) !== 0;
										break;
									case"ends":
										$check = ( strpos( $val, $fval ) + strlen( $fval ) ) == strlen( $val );
										break;
									case"not_ends":
										$check = ( strpos( $val, $fval ) + strlen( $fval ) ) != strlen( $val );
										break;
									case"less":
										$check = (float) $val < (float) $fval;
										break;
									case"greater":
										$check = (float) $val > (float) $fval;
										break;
									case"less_date":
										$check = strtotime( $val, $time ) < strtotime( $fval, $time );
										break;
									case"greater_date":
										$check = strtotime( $val, $time ) > strtotime( $fval, $time );
										break;
			...
			public function verify_log( $feed ){
				$link               = "N/A";
				$desc               = __( "Added to ", 'contact-form-hubspot-crm' );
			...
			public function entry_info_send_checkbox( $form_id, $lead ){
				...
				if( ! $this->show_send_to_crm_button() ){
					return;
				}
				
				// If this is not the Edit screen, get outta here.
				if( empty( $_POST["screen_mode"] ) || $_POST["screen_mode"] === 'view' ){
					return;
				}
				...
				if( apply_filters( $this->id . '_show_manual_export_button', true ) ){
					printf(
						'<input type="checkbox" name="' . $this->id . '_update" id="' . $this->id . '_update" value="1" /><label for="' . $this->id . '_update" title="%s">%s</label><br /><br />',
						__( 'Create or update this entry in HubSpot. The fields will be mapped according to the form feed settings.', 'contact-form-hubspot-crm' ),
						__( 'Send to HubSpot', 'contact-form-hubspot-crm' ) );
				} else{
			...
			public function entry_info_send_button( $button = '' ){
				...
				if( $mode === 'view' ){
					...
					$button .= '<input type="submit" class="button button-large button-secondary alignright" name="' . $this->id . '_send" style="margin-left:5px; ' . $margin . '" title="' .
					           __( 'Create or update this entry in HubSpot. The fields will be mapped according to the form feed settings.', 'contact-form-hubspot-crm' ) . '" value="' .
					           __( 'Send to HubSpot', 'contact-form-hubspot-crm' ) . '" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
					//logs button
					...
					$button  .= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="' .
					            __( 'Go to HubSpot Logs', 'contact-form-hubspot-crm' ) . '" href="' . $log_url . '">' . __( 'HubSpot Logs', 'contact-form-hubspot-crm' ) . '</a>';
				...
				if( isset( $info['info'] ) && is_array( $info['info'] ) ){
					if( $is_html ){
						if( isset( $info['info_title'] ) ){
							$str .= '<tr><td style="font-family: Helvetica, Arial, sans-serif;background-color: #C35050; height: 36px; color: #fff; font-size: 24px; padding: 0px 10px">' .
							        $info['info_title'] . '</td></tr>' . "\n";
						}
						if( is_array( $info['info'] ) && count( $info['info'] ) > 0 ){
							...
							foreach( $info['info'] as $f_k => $f_val ){
								$str .= '<tr><td style="padding-top: 10px;color: #303030;font-family: Helvetica;font-size: 13px;line-height: 150%;text-align: right; font-weight: bold; width: 28%; padding-right: 10px;">' .
								        $f_k . '</td><td style="padding-top: 10px;color: #303030;font-family: Helvetica;font-size: 13px;line-height: 150%;text-align: left; word-break:break-all;">' .
								        $f_val . '</td></tr>' . "\n";
							}
							$str .= "</table></td></tr>";
						}
					...
					foreach( $feeds_meta as $feed ){
						...
						if( in_array( $event, array( 'restore', 'update', 'delete', 'add_note', 'delete_note' ) ) ){
							...
							if( $event == 'add_note' && ! empty( self::$note ) ){
								$temp                     = array(
									'Title'    => array( 'value' => self::$note['title'] ),
									'Body'     => array( 'value' => self::$note['body'] ),
									'ParentId' => array( 'value' => $feed['crm_id'] )
								);
								$parent_id                = self::$note['id'];
						...
						if( $this->post( 'id', $res ) != "" ){
							...
							if( $action == "Added" ){
								...
								if( empty( $res['link'] ) ){
									$msg = sprintf(
										__( 'Successfully Updated to HubSpot (%s) with ID # %s . View entry at %s', 'contact-form-hubspot-crm' ),
										$feed['object'],
										$res['id'],
										$res['link'] );
								} else{
			...
			public function process_tags( $entry, $value ){
				...
				if( ! empty( $matches[0] ) ){
					...
					foreach( $matches[0] as $m ){
						...
						if( is_array( $val_cust ) ){
							$val_cust = trim( implode( ' ', $val_cust ) );
						}
						$vals[ '{' . $m . '}' ] = $val_cust;
					}
