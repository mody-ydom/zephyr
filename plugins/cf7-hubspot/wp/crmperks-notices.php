	...
	if( ! class_exists( 'vx_crmperks_notice_vxcf_hubspot' ) ):
		...
		class vx_crmperks_notice_vxcf_hubspot extends vxcf_hubspot{
				...
				if( ! empty( $_GET['page'] ) && $_GET['page'] == $this->id ){
					...
					if( empty( $show ) ){
						...
						if( ! class_exists( 'vxcf_form' ) ){
							...
							$msg = sprintf(
								__(
									'Want to save conatct form 7 submissions? Manage contact form 7 entries , view HubSpot status of any entry and Resend any entry to HubSpot with free %sContact Form Entries Plugin%s',
									'crm-perks-forms' ),
								'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">',
								'</a>' );
							?>
			...
			public function install_forms_notice_bottom(){
				...
				if( ! empty( $_GET['page'] ) && $_GET['page'] == $this->id ){
					...
					if( ! class_exists( 'vxcf_form' ) && ! file_exists( WP_PLUGIN_DIR . '/' . $plugin_file ) ){
						...
						$msg = sprintf(
							__(
								'Want to save conatct form 7 submissions? Manage contact form 7 entries, View and Resend any entry to Hubspot with free %sConatct Form Entries Plugin%s',
								'crm-perks-forms' ),
							'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">',
							'</a>' );
						?>
			...
			public function plugin_msgs( $file, $data, $status ){
              ...
              <tr class="plugin-update-tr">
                <td colspan="5" class="plugin-update">
                  ...
                  <div style="background-color: rgba(224, 224, 224, 0.5);  padding: 5px; margin: 0px 10px 10px 28px ">
			...
			public function admin_footer( $text ){
				if( isset( $_GET['page'] ) && $_GET['page'] == $this->id ){
					$text = sprintf(
						__( 'if you enjoy using %s, please %s leave us a %s rating%s. A %shuge%s thank you in advance.', 'contact-form-entries' ),
						'<b>' . self::$title . '</b>',
				...
				if( ! empty( $install_time ) && is_array( $install_time ) && ! empty( $install_time['time'] ) && empty( $install_time['review_closed'] ) ){
					...
					if( $time > $time_i ){
                      ...
                      <div class="notice notice-info is-dismissible vxcf-review-notice" style="margin: 14px 0 -4px 0">
                        <p><?php echo sprintf(
								__(
									'You\'ve been using Contact Form 7 hubspot Plugin for some time now; we hope you love it!.%s If you do, please %s leave us a %s rating on WordPress.org%s to help us spread the word and boost our motivation.',
									'contact-form-entries' ),
								'<br/>',
								'<a href="' . $this->review_link . '" target="_blank" rel="noopener noreferrer">',
								'&#9733;&#9733;&#9733;&#9733;&#9733;',
								'</a>' ); ?></p>
                        <p><a href="<?php echo $this->review_link ?>" class="vxcf_close_notice_a" target="_blank" rel="noopener noreferrer"><?php _e(
									'Yes, you deserve it',
									'contact-form-entries' ) ?></a> | <a href="#" class="vxcf_close_notice_a"><?php _e(
									'Dismiss this notice',
									'contact-form-entries' ); ?></a></p>
                      </div>
			...
			public function notice(){
              ...
              <div class="updated below-h2 vx_pro_version" style="border-left-color: #1192C1; margin: 30px 20px 30px 0px">
                ...
                <p><i class="fa fa-check"></i> Verify lead's phone number and get detailed information about phone number using phone lookup apis, We support many good phone lookup apis like
                  everyoneapi, Clearbit api , whitepages api , twilio api and numverify api.</p>
			...
			public function free_plugins_notice(){
              ...
              <div class="updated below-h2" style="border: 1px solid  #1192C1; border-left-width: 6px; padding: 5px 12px;">
                ...
                <p><b><a href="https://wordpress.org/plugins/crm-perks-forms/" target="_blank">CRM Perks Forms</a></b> is lightweight and highly optimized contact form builder with Poups and floating
                  buttons.</p>
                
                <p><b><a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">Contact Form Entries</a></b> saves contact form submissions from all popular contact forms(contact
                  form 7 , crmperks forms, ninja forms, Gravity forms etc) into database.</p>
                ...
                <p><b><a href="https://www.crmperks.com/plugins/woocommerce-plugins/woocommerce-hubspot-plugin/" target="_blank">WooCommerce HubSpot Plugin</a></b> allows you to quickly integrate
                  WooCommerce Orders with Hubspot CRM.</p>
