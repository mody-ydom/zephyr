...
<div class="vx_wrap">
  <div>
    <h2 class="vx_img_head">
      <img alt="<?php _e( "HubSpot Feeds", 'contact-form-hubspot-crm' ) ?>" title="<?php _e( "HubSpot Feeds", 'contact-form-hubspot-crm' ) ?>"
           src="<?php echo $this->get_base_url() ?>images/hubspot-crm-logo.png?ver=1"/> <?php _e( "HubSpot Log", 'contact-form-hubspot-crm' ); ?> <select
        style="min-width: 120px; max-width: 200px; margin-left: 12px; font-weight: normal;" id="vx_sel_feed">
  ...
  <div>
    ...
    <div style="float: right;">
      <form id="vx_form" class="crm_form" method="get">
        <div>
          ...
          <span style="<?php if( self::post( 'time' ) != "custom" ){
			  echo "display:none";
		  } ?>" class="crm_custom_range">
  <input type="text" name="start_date" placeholder="<?php _e( 'From Date', 'contact-form-hubspot-crm' ) ?>From Date" value="<?php if( isset( $_REQUEST['start_date'] ) ){
	  echo $this->post( 'start_date' );
  } ?>" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="<?php if( isset( $_REQUEST['end_date'] ) ){
	  echo $this->post( 'end_date' );
  } ?>" placeholder="<?php _e( 'To Date', 'contact-form-hubspot-crm' ) ?>" name="end_date" style="width: 100px">
  </span>
          
          <button type="submit" title="<?php _e( 'Search', 'contact-form-hubspot-crm' ) ?>" name="search" class="button-secondary button crm_input_inline"><i class="fa fa-search"></i> <?php _e(
				  'Search',
    ...
    <form method="post">
      ...
      <div class="crm_actions tablenav">
        <div class="alignleft actions">
          ...
          <button type="submit" class="button-secondary button crm_input_inline" title="<?php _e( 'Apply', 'contact-form-hubspot-crm' ) ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php _e(
				  'Apply',
				  'contact-form-hubspot-crm' ) ?></button>
			...
			<?php
				...
				if( $items > 0 ){
					...
					?>
                  <button type="button" name="tab_action" title="<?php _e( 'Export as CSV', 'contact-form-hubspot-crm' ) ?>" id="vx_export" class="button-secondary button crm_input_inline vx_left_10">
                    <i class="fa fa-download"></i> <?php _e( 'Export as CSV', 'contact-form-hubspot-crm' ) ?></button>
					<?php
				}
				...
				if( $_log_id != "" ){
					if( isset( $data['feeds'][0]['entry_id'] ) && $data['feeds'][0]['entry_id'] != "" ){
						...
						?>
                    <a href="<?php echo $log_link . '&entry_id=' . $entry_id; ?>" title="<?php echo sprintf( __( 'View Entry# %s Logs', 'contact-form-hubspot-crm' ), $entry_id ); ?>"
                       class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php echo sprintf( __( 'View Entry# %s Logs', 'contact-form-hubspot-crm' ), $entry_id ); ?></a><?php
					}
				}
				if( $this->post( 'entry_id' ) != "" || $_log_id != "" ){
					?><a href="<?php echo $log_link; ?>" title="<?php _e( 'View All Logs', 'contact-form-hubspot-crm' ) ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php _e(
					'View All Logs',
		  ...
		  <?php
			  if( $items > 0 ){
				  ?>
                <div class="tablenav-pages"><span id="paging_header" class="displaying-num"><?php _e( 'Displaying', 'contact-form-hubspot-crm' ) ?> <span
                      id="paging_range_min_header"><?php echo $data['min'] ?></span> - <span id="paging_range_max_header"><?php echo $data['max'] ?></span> of <span
                      id="paging_total_header"><?php echo $data['items'] ?></span></span><?php echo $data['links'] ?></div>
				  <?php
			  }
		  ?>
      </div>
      ...
      <table class="widefat fixed sort" cellspacing="0">
		...
		<?php
			if( is_array( $data['feeds'] ) && ! empty( $data['feeds'] ) ){
				...
				foreach( $data['feeds'] as $feed ){
					...
					?>
                  <tr class='author-self status-inherit <?php if( in_array( $row['id'], $log_ids ) ){
					  echo 'vx_yellow ';
				  }
					  echo $sno % 2 == 0 ? 'alternate' : '' ?>' id="tr_<?php echo $row['id'] ?>" data-id="<?php echo $row['id'] ?>">
                    <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo $row['id'] ?>" class="crm_input_check"></td>
                    <td class="vx_icon_col">
                      <img src="<?php echo $base_url ?>images/<?php echo $row["status_img"] ?>.png"
                           alt="<?php echo $row["status"] ? __( "Active", 'contact-form-hubspot-crm' ) : __( "Inactive", 'contact-form-hubspot-crm' ); ?>" title="<?php echo $row['title']; ?>"
                           class="crm_status_img"/>
                    </td>
                  ...
                  <tr style="display: none;">
                    <td colspan="8" class="entry_detail"></td>
                  </tr>
					<?php
				}
			} else{
		...
		<?php
			if( $items > 0 ){
				?>
              <div class="crm_actions tablenav">
                <a id="vx_clear_logs" class="button" title="<?php _e( 'Clear HubSpot Log', 'contact-form-hubspot-crm' ) ?>" href="<?php echo wp_nonce_url(
					admin_url( 'admin.php?page=' . $this->post( 'page' ) . "&view=log&vx_tab_action_" . $this->id . "=clear_logs" ),
					'vx_nonce',
					'vx_nonce' ); ?>"><?php _e( 'Clear HubSpot Log', 'contact-form-hubspot-crm' ) ?></a>
                <div class="tablenav-pages"><span id="paging_header" class="displaying-num"><?php _e( 'Displaying', 'contact-form-hubspot-crm' ) ?> <span
                      id="paging_range_min_header"><?php echo $data['min'] ?></span> - <span id="paging_range_max_header"><?php echo $data['max'] ?></span> of <span
                      id="paging_total_header"><?php echo $data['items'] ?></span></span><?php echo $data['links'] ?></div>
              </div>
