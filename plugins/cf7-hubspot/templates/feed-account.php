...
<div class="vx_wrap">
  <h2 class="vx_img_head">
    <img alt="<?php _e( "HubSpot Feeds", 'contact-form-hubspot-crm' ) ?>" title="<?php _e( "HubSpot Feeds", 'contact-form-hubspot-crm' ) ?>"
         src="<?php echo $this->get_base_url() ?>images/hubspot-crm-logo.png?ver=1"/> <?php
	...
	<?php
		...
		if( ! empty( $info ) && ! $this->api_is_valid( $info ) ){
			?>
          <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(
				  sprintf(
					  __( "We are unable to login to HubSpot with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'contact-form-hubspot-crm' ),
					  "<a href='" . $this->link_to_settings() . "'>",
					  "</a>" ) ); ?></div>
			<?php
			// return;
		}
  ...
  <div class="vx_contents">
    ...
    <form method="post" action="">
      ...
      <div id="crm_object_div" style="<?php if( empty( $info ) || empty( $form_id ) ){
		  echo 'display:none';
	  } ?>">
		  <?php
      ...
      if ($('.vx_required').length) {
        ...
        $('.vx_required').each(function () {
          ...
          if (value == '') {
            ...
            attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php _e(
				'This is a required field',
				'contact-form-hubspot-crm' ) ?></div></div>');
            return false;
          }
        });
      }
...
//}   
    ...
    function load_fields(refresh) {
      ...
      button_state_vx('ajax', button);
      vx_ajax = $.post(ajaxurl, {
        action: 'get_field_map_<?php echo $this->id ?>',
        form_id: form_id,
        account: account,
        module: module,
        id: id,
