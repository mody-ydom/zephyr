		...
		class ACF_To_REST_API_Settings {
			...
			public static function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data ) {
				if ( isset( $plugin_data['slug'] ) && 'acf-to-rest-api' == $plugin_data['slug'] ) {
					...
					$plugin_meta['acf_to_rest_api_donation'] = sprintf( '<a href="%s" target="_blank" class="acf-to-rest-api-donation-button"><span class="dashicons dashicons-heart"></span> %s</a>',
					                                                    ACF_To_REST_API_Donation::DONATION_URL,
					                                                    esc_html__( 'Make a donation', 'acf-to-rest-api' ) );
				}
			...
			public static function acf_admin_setting() {
				add_settings_section( 'acf_to_rest_api_settings_section',
				                      __( 'ACF to REST API', 'acf-to-rest-api' ),
				                      array( __CLASS__, 'add_settings_section' ),
				                      'permalink' );
				
				add_settings_field( 'acf_to_rest_api_request_version',
