...
$lead[$k]=$files;
       ...
       $all_values=array_merge($all_values,$extra_values);
			...
			public static function display_msg( $type, $message, $id = "" ){
				...
				if( $type == "admin" ){
					?>
                  <div class="error vx_notice below-h2 notice is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e(
								  'Contact Form Entries Plugin',
								  'contact-form-entries' ) ?>. </b> <?php echo wp_kses_post( $message ); ?> </p>
                  </div>
					<?php
				} else{
					?>
					...
					case'na':
						if( class_exists( 'Ninja_Forms' ) ){
							...
							foreach( $form_fields as $obj ){
								...
								if( in_array( $type, array( 'spam', 'confirm' ) ) || ! isset( $field['required'] ) ){
									continue;
								}
								if( $type == 'checkbox' ){
