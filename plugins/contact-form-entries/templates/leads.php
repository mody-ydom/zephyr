...
<div class="vx_wrap">
				...
				foreach( $counts as $label => $count ){
                  ...
                  <li>
					  <?php
						  ...
						  $k ++;
					  ?>
                    <a href="<?php echo $entries_link_form . '&' . $field . '=' . $val ?>" class="<?php echo $current ?>" title="<?php echo ucfirst( $label ) ?>"><?php echo ucfirst( $label ) ?> <span
                        class="count">(<?php echo $count ?>)</span></a>
                  </li>
					<?php
				}
      ...
      <div style="float: right;">
        <form id="vx_form" class="crm_form" method="get">
          <div>
            ...
            <span style="<?php if( vxcf_form::post( 'time' ) != "custom" ){
				echo "display:none";
			} ?>" class="crm_custom_range">
  <input type="text" name="start_date" placeholder="<?php _e( 'From Date', 'contact-form-entries' ) ?>" value="<?php if( isset( $_REQUEST['start_date'] ) ){
  ...
  <input type="text" class="vxc_date crm_input_inline" value="<?php if( isset( $_REQUEST['end_date'] ) ){
	  echo $_REQUEST['end_date'];
            ...
            <button type="submit" title="<?php _e( 'Search', 'contact-form-entries' ) ?>" class="button-secondary button crm_input_inline"><i class="fa fa-search"></i> <?php _e(
					'Search',
					'contact-form-entries' ) ?></button>
    ...
    <form method="post">
      ...
      <div class="crm_actions tablenav">
        <div class="alignleft actions">
          ...
          <button type="submit" class="button-secondary button crm_input_inline" title="<?php _e( 'Apply', 'contact-form-entries' ) ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php _e(
				  'Apply',
				  'contact-form-entries' ) ?></button>
			...
			<?php
				if( $items > 0 ){
					?>
                  <button type="button" name="tab_action" title="<?php _e( 'Export as CSV', 'contact-form-entries' ) ?>" id="vx_export" class="button-secondary button crm_input_inline vx_left_10"><i
                      class="fa fa-download"></i> <?php _e( 'Export as CSV', 'contact-form-entries' ) ?></button>
					<?php
				}
		  ...
		  <?php
			  if( $items > 0 ){
				  ?>
                <div class="tablenav-pages"><span id="paging_header" class="displaying-num"><?php _e( 'Displaying', 'contact-form-entries' ) ?> <span
          ...
          <th scope="col" class="manage-column vx_col"></th>
			<?php
				...
				foreach( $fields as $field ){
					...
					?>
                  <th scope="col" class="manage-column vx_sort_sql vx_sort column-<?php echo $field['_id'] . $hide_col; ?>" data-sort="string" id="<?php echo $field['_id']; ?>"
                      data-name="<?php echo $field['name']; ?>" <?php if( isset( $field['vx_width'] ) ){
					  echo 'style="width: ' . $field['vx_width'] . '"';
				  } ?> ><?php echo $field['label'] ?>
                    <i class="fa fa-caret-<?php if( $sel ){
						echo $order_icon;
					} else{
						echo $crm_order;
					} ?> vx_sort_icon <?php if( ! $sel ){
						echo 'vx_hide_sort';
					} ?>"></i>
                  </th>
					<?php
		...
		<?php
			...
			if( is_array( $leads ) && ! empty( $leads ) ){
				...
				foreach( $leads as $lead ){
					$sno ++;
					?>
                  <tr class='<?php echo 'vx_lead_' . $lead['type'];
					  if( $lead['is_read'] == 0 ){
						  echo ' vx_lead_unread';
					  }
					  if( $lead['type'] == '1' ){
									...
									if( $f_no == 1 ){
                                      ...
                                      <div class="row-actions">
										  <?php
											  if( $status == 'trash' ){
												  ?>
                                                <span class="edit"><a href="<?php echo $restore_link ?>" title="<?php _e( 'Restore', 'contact-form-entries' ) ?>"><?php _e(
															'Restore',
															'contact-form-entries' ) ?></a> | </span>
                                                
                                                <span class="trash"><a class="submitdelete" title="<?php _e( 'Delete', 'contact-form-entries' ) ?>" href="<?php echo $delete_link ?>"><?php _e(
															'Delete',
															'contact-form-entries' ) ?></a> </span>
												  <?php
											  } else{
												  ?>
                                                <span class="edit"><a href="<?php echo $entry_link ?>" title="<?php _e( 'View', 'contact-form-entries' ) ?>"><?php _e(
															'View',
															'contact-form-entries' ) ?></a> | </span>
                                                
                                                <!--span class="inline"><a href="<?php echo $mark_link ?>"  title="<?php echo $mark_label ?>"><?php echo $mark_label ?></a> | </span--->
                                                
                                                <span class="trash"><a class="submitdelete" title="<?php _e( 'Trash', 'contact-form-entries' ) ?>" href="<?php echo $trash_link ?>"><?php _e(
															'Trash',
															'contact-form-entries' ) ?></a> </span>
												  <?php
											  }
  ...
  <div id="vx_sel_cols_html">
    ...
    <form method="post" id="vx_fields_form">
			  ...
			  if( ! empty( $fields_arr ) && is_array( $fields_arr ) ){
				  foreach( $fields_arr as $field ){
                    ...
                    <div class="vx_td_border_bottom">
                      <label for='vx_check_<?php echo $field['name'] ?>'><input type="checkbox" <?php if( isset( $entry_fields[ $field['name'] ] ) ){
      ...
      <div style="padding: 20px 20px 0px 20px;">
        ...
        &nbsp; &nbsp; &nbsp;<button type="button" title="<?php _e( 'Cancel', 'contact-form-entries' ) ?>" name="<?php echo vxcf_form::$id ?>_cancel" id="vx_colorbox_close" class="button"><i
            class="fa fa-times"></i> <?php _e( 'Cancel', 'contact-form-entries' ) ?></button>
      </div>
    </form>
  </div>
