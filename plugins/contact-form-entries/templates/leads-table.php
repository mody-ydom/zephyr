	...
	if( ! empty( $search ) ){
      ...
      <p class="vx_table_actions">
        <input type="search" placeholder="<?php _e( 'Search', 'contact-form-entries' ); ?>" data-column="all" class="vx_search">
		  <?php
			  if( ! empty( $export ) ){ ?><a class="vx_export_btn" href="?vx_crm_form_action=download_csv&vx_crm_key=<?php echo $export ?>"><?php _e(
				  'Download CSV',
				  'contact-form-entries' ); ?></a> <?php } ?>
      </p>
		<?php
	}
?>
  ...
  <tr>
	  <?php
		  if( ! empty( $atts['number-col'] ) ){
			  ?>
            <th <?php if( ! empty( $atts['number-col-width'] ) ){
