...
<div class="vx_wrap">
  <h2 class="vx_img_head"><?php echo $this->entry_title;
	  ...
	  ?>
    <a class="button button-default" style="vertical-align: middle" href="<?php echo $form_link; ?>"><i class="fa fa-reply"></i> <?php echo sprintf(
			__( 'Back to %s', 'contact-form-entries' ),
			$this->entry_title_small ) ?></a>
  </h2>
                            ...
                            <div class="vx_edit">
								<?php
									if( in_array( $type, array( 'textarea', 'chat', 'comma' ) ) ){
										...
										?>
                                      <textarea id="vx_<?php echo $field['name']; ?>" class="vx_input vx_input_100" name="lead[<?php echo $f_name ?>]" <?php echo $req ?>><?php echo htmlentities(
											  $value ); ?></textarea>
										<?php
										$value = nl2br( $value );
                                      ...
                                      <div class="vx_file_single">
										...
										?>
                                      <input type="<?php echo $type ?>" id="vx_<?php echo $field['name']; ?>" <?php echo $req ?> class="vx_input vx_input_100 <?php echo $date_class ?>"
                                             name="lead[<?php echo $f_name ?>]" autocomplete="off" value="<?php echo htmlentities( $value ); ?>">
										<?php
									}
									$type = $field['type'];
								?>
								...
								<?php
									...
									if( $type == 'email' && filter_var( $value, FILTER_VALIDATE_EMAIL ) ){ //it is a url
                                      ...
                                      <a href="<?php echo $value ?>" target="_blank">
                                        <img src="<?php echo $value ?>" class="vx_img_thumb_field"/>
                                      </a>
										<?php
          ...
          <div class="vx_row">
			...
			<?php
				...
				if( ! empty( $lead['user_id'] ) ){
					...
					$user_link = '<a href="' . get_edit_user_link( $lead['user_id'] ) . '" title="' . __( 'Entry Submitted By', 'contact-form-entries' ) . '">' . esc_attr( $userdata->display_name ) .
					             '</a>';
					?>
                  <div class="vx_row">
                    <div class="vx_col3">
			...
			<?php
				if( $lead['status'] == 1 ){
                  ...
                  <div id="vx_edit_div">
                    ...
                    <a href="<?php echo $del_link; ?>" title="<?php _e( 'Delete Permanently', 'contact-form-entries' ) ?>" class="vx_del_link vx_del_lead vx_float_right"><?php _e(
							'Delete Permanently',
							'contact-form-entries' ) ?></a>
                  </div>
					<?php
				} else{
					?>
                  <div class="vx_edit_div">
                    <button type="button" class="button button-primary" id="vx_edit_button" title="<?php _e( 'Edit', 'contact-form-entries' ) ?>"><i class="fa fa-pencil"></i> <?php _e(
							'Edit',
							'contact-form-entries' ) ?></button>
                    
                    <a href="<?php echo $trash_link; ?>" style="line-height: 26px;" title="<?php _e( 'Trash', 'contact-form-entries' ) ?>" class="vx_del_link vx_del_lead vx_float_right"><?php _e(
							'Trash',
							'contact-form-entries' ) ?></a>
                  </div>
                  
                  <div class="vx_save_div" style="display: none;">
                    <button type="submit" autocomplete="off" name="<?php echo vxcf_form::$id ?>_submit" value="yes" class="button button-primary" id="vx_save_lead"
                            title="<?php _e( 'Save', 'contact-form-entries' ) ?>">
                    ...
                    <button type="button" id="vx_cancel_button" title="<?php _e( 'Cancel', 'contact-form-entries' ) ?>" class="button button-default  vx_float_right"><i
...
<script type="text/javascript">
  ...
  jQuery(document).ready(function ($) {
    ...
    $('#vx_en_form').submit(function () {
	  ...
	  if(! empty( $date_field )){
	  ?>
    $(".<?php echo trim( $date_field ) ?>").datepicker({
      changeMonth: true,
      changeYear: true,
    ...
    $(document).on('change', '.vx_note_email', function () {
      ...
      if ($(this).val() == '') {
        note.find('.vx_note_subject').hide();
      }
    ...
    $(document).on('click', '.add_note_btn', function () {
      ...
      var arr = {
        action: 'actions_<?php echo vxcf_form::$id ?>',
        entry_id: vx_entry_id,
        note: note,
        note_color: note_color,
        action2: 'add_note',
        note_email: note_email,
        note_subject: note_subject,
        vx_crm_ajax: vx_crm_ajax,
      };
