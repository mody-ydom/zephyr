			...
			public function note_template( $note ){
              ...
              <div class="crm_note vx_note_temp <?php echo $note_color ?>" data-id="<?php echo $note['id'] ?>" data-color="<?php echo $note['color'] ?>">
                ...
                <div class="crm_arrow_left">
                  ...
                  <div class="key_info">
                    <span class="posted_by" title="<?php _e( 'Note created by', 'contact-form-entries' ); ?>"><i class="fa fa-user"></i> <span
                        class="post_user"><?php echo $note['display_name'] ?></span></span>
                    
                    <span class="post_time" title="<?php _e( 'Note created at', 'contact-form-entries' ); ?>"><i class="fa fa-clock-o"></i> <span class="date_time"><?php echo date(
								"d/M/y H:i:s",
								strtotime(
									$note['created'] ) ) ?></span></span>
					  <?php
						  if( ! empty( $note['email'] ) ){
							  ?>
...
<i class="fa fa-envelope"></i> <?php echo $note['email']; ?></span>
                    ...
                    <span class="del_note vx_del_link" data-id="<?php echo $note['id'] ?>"> <a href="#" title="<?php _e( "Delete", 'contact-form-entries' ); ?>" class="reg_ok"><?php _e(
								"Delete",
								'contact-form-entries' ); ?> </a><span class="reg_proc" style="display: none;"><i class="fa fa-circle-o-notch fa-spin"></i> <?php _e(
								"Deleting ...",
								'contact-form-entries' ); ?></span></span>
                  </div>
                </div>
			...
			public function crmperks_forms_duplicate_field( $id, $field ){
				?>
              <div class="cfx_field_dup_div cfx_field_input cfx_field_row" style="<?php if( in_array( $field['type'], array( 'file', 'html', 'hr', 'star', 'range', 'hidden' ) ) ){
				  echo 'display:none;';
			  } ?>">
                <label class="crm_text_label"><input type="checkbox" value="yes" class="valid_err_check" name="fields[<?php echo $id; ?>][dup_check]"
                                                     data-name="dup_check" <?php if( isset( $field['dup_check'] ) && $field['dup_check'] == "yes" ) echo 'checked="checked"' ?>> <?php _e(
						'No Duplicates - ',
						'contact-form-entries' ) ?>   </label> <a href="javascript:void(0);" onclick="sf_colorbox('<?php _e(
					  'No Duplicate Fields Explanation',
					  'contact-form-entries' ) ?>','#sf_duplicate_fields_help');"><?php _e( 'Help', 'contact-form-entries' ) ?></a>
                
                <div class="valid_err_div" style="<?php if( empty( $field['dup_check'] ) ){
					echo "display:none";
				} ?>">
                  ...
                  <div class="crm-panel-description">
					  <?php _e(
						  'This message will be displayed to the visitor if duplicate found. You can use %field_value% to display the field value submitted by user.',
						  'contact-form-entries' ) ?></div>
                  <input type="text" name="fields[<?php echo $id; ?>][valid_err_msg]" data-name="valid_err_msg" placeholder="Enter No Duplicates Validation Error Message" class="text"
                         value="<?php echo $field['valid_err_msg']; ?>"/></div>
              </div>
				<?php
			}
			...
			public function forms_stats_table(){
				...
				if( ! empty( $forms ) ){
                  ...
                  <thead>
                  <tr>
                    <th style="width: 55%;"><?php _e( 'Form Title', 'contact-form-entries' ) ?></th>
			...
			public function add_forms_stats(){
              ...
              <div class="crm_panel">
                <div class="crm_panel_head">
                  <div class="crm_head_div"><span class="crm_head_text"><?php _e( 'Entries', 'contact-form-entries' ) ?></span></div>
                  <div class="crm_btn_div" title="Expand / Collapse"><i class="fa fa-minus crm_toggle_btn"></i></div>
                  <div class="crm_clear"></div>
                </div>
			...
			public function crmperks_disable_entry_check( $options ){
              ...
              <div class="crm-panel-field">
                <label><input type="checkbox" name="vx_config[disable_db]" value="yes" <?php if( ! empty( $options['disable_db'] ) ){
              ...
              <div class="wrap">
                ...
                <h2 class="nav-tab-wrapper">
					<?php
						...
						foreach( $tabs as $k => $v ){
							?>
                          <a href="<?php echo $link . $k ?>" class="nav-tab <?php if( $k == $view ){
							  echo 'nav-tab-active';
						  } ?>"><?php echo $v; ?></a>
			...
			public function settings_page(){
				...
				if( ! empty( $_POST[ vxcf_form::$id . "_uninstall" ] ) ){
					...
					$uninstall_msg = sprintf(
						__( "Contact Form Entries Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-entries' ),
						"<a href='plugins.php'>",
						"</a>" );
				...
				if( ! empty( $_POST[ vxcf_form::$id . '_submit' ] ) && ! empty( $_POST['lead'] ) && ! empty( $form_id ) ){
					...
					if( ! empty( $_POST['vxcf_leads_submit'] ) ){
						...
						if( $is_valid ){
							...
							$msgs['save'] = array(
								'class' => 'updated',
								'msg'   => sprintf( __( "Entry Updated. %sGo back to entries%s", 'contact-form-entries' ), '<a href="' . $feed_link . '">', "</a>" )
							);
						} else{
