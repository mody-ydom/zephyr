<?php
	...
	function duplicate_post_get_edit_or_view_link( $post ) {
		...
		if ( $can_edit_post && 'trash' != $post->post_status ) {
			return sprintf( '<a href="%s" aria-label="%s">%s</a>',
			...
			if ( in_array( $post->post_status, array( 'pending', 'draft', 'future' ) ) ) {
				if ( $can_edit_post ) {
					...
					return sprintf( '<a href="%s" rel="bookmark" aria-label="%s">%s</a>',
				...
				return sprintf( '<a href="%s" rel="bookmark" aria-label="%s">%s</a>',
					...
					/* translators: %s: post title */ esc_attr( sprintf( __( 'View &#8220;%s&#8221;', 'default' ), $title ) ),
					            $title );
			}
		}
	...
	function duplicate_post_admin_bar_render() {
		...
		if ( ! empty( $current_object ) ) {
			if ( ! empty( $current_object->post_type ) && ( $post_type_object = get_post_type_object( $current_object->post_type ) ) && duplicate_post_is_current_user_allowed_to_copy() &&
			     ( $post_type_object->show_ui || 'attachment' == $current_object->post_type ) && ( duplicate_post_is_post_type_enabled( $current_object->post_type ) ) ) {
			...
			$id   = $_GET['post'];
			$post = get_post( $id );
			if ( ! is_null( $post ) && duplicate_post_is_current_user_allowed_to_copy() && duplicate_post_is_post_type_enabled( $post->post_type ) ) {
				$wp_admin_bar->add_menu( array(
					                         'id'    => 'new_draft',
					                         'title' => esc_attr__( "Copy to a new draft", 'duplicate-post' ),
		...
		if ( is_admin() ) {
			...
			if ( ! is_null( $current_screen ) ) {
				if ( 'edit' === $current_screen->base ) {
					...
					if ( duplicate_post_is_current_user_allowed_to_copy() && duplicate_post_is_post_type_enabled( $post_type ) ) {
						duplicate_post_enqueue_css();
					}
				}
			}
