...
';
	...
	function duplicate_post_quick_edit_remove_original( $column_name, $post_type ) {
		...
		printf( '<fieldset class="inline-edit-col-left" id="duplicate_post_quick_edit_fieldset">
			<div class="inline-edit-col">
		        ...
		        __( 'Delete reference to original item.',
		            'duplicate-post' ),
		        __( 'The original item this was copied from is: <span class="duplicate_post_original_item_title_span"></span>',
		            'duplicate-post' ) );
	}
	...
	function duplicate_post_add_custom_box() {
		...
		foreach ( $screens as $screen ) {
			add_meta_box( 'duplicate_post_show_original',           // Unique ID
		...
		if ( $original_item ) {
          ...
          <p id="duplicate-post-remove-original-description">
			  <?php
				  ...
				  printf( __( 'The original item this was copied from is: <span class="duplicate_post_original_item_title_span">%s</span>', 'duplicate-post' ),
	...
	function duplicate_post_make_duplicate_link_row( $actions, $post ) {
		...
		if ( apply_filters( 'duplicate_post_show_link', duplicate_post_is_current_user_allowed_to_copy() && duplicate_post_is_post_type_enabled( $post->post_type ), $post ) ) {
			$actions['clone'] = '<a href="' . duplicate_post_get_clone_post_link( $post->ID, 'display', false ) . '" aria-label="' .
			                    esc_attr( /* translators: %s: Post title. */ sprintf( __( 'Clone &#8220;%s&#8221;', 'duplicate-post' ), $title ) ) . '">' .
			...
			$actions['edit_as_new_draft'] = '<a href="' . duplicate_post_get_clone_post_link( $post->ID ) . '" aria-label="' .
			                                esc_attr( /* translators: %s: Post title. */ sprintf( __( 'New draft of &#8220;%s&#8221;', 'duplicate-post' ), $title ) ) . '">' .
   ...
   */
	...
	function duplicate_post_add_plugin_links( $links, $file ) {
		if ( $file == plugin_basename( dirname( __FILE__ ) . '/duplicate-post.php' ) ) {
			$links[] = '<a href="https://yoast.com/wordpress/plugins/duplicate-post/" aria-label="' . esc_attr__( 'Documentation for Duplicate Post', 'duplicate-post' ) . '">' .
			           esc_html__( 'Documentation', 'duplicate-post' ) . '</a>';
		}
	...
	function duplicate_post_action_admin_notice() {
		if ( ! empty( $_REQUEST['cloned'] ) ) {
			...
			printf( '<div id="message" class="notice notice-success fade"><p>' . /* translators: %s: number */ _n( '%s item copied.',
			                                                                                                       '%s items copied.',
			                                                                                                       $copied_posts,
			                                                                                                       'duplicate-post' ) . '</p></div>',
			        $copied_posts );
	...
	function duplicate_post_action_handler( $redirect_to, $doaction, $post_ids ) {
		...
		foreach ( $post_ids as $post_id ) {
			...
			if ( ! empty( $post ) ) {
				if ( get_option( 'duplicate_post_copychildren' ) != 1 || ! is_post_type_hierarchical( $post->post_type ) ||
