	...
	class WP_Import extends WP_Importer {
		...
		function header() {
			...
			if ( isset( $updates[ $basename ] ) ) {
				...
				printf( __( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'wordpress-importer' ),
			...
			foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
				...
				if ( ! empty( $_POST['user_map'][ $i ] ) ) {
					...
					if ( ! is_wp_error( $user_id ) ) {
						...
						printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'wordpress-importer' ),
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( is_wp_error( $remote_response ) ) {
				...
				return new WP_Error( 'import_file_error',
				                     sprintf( /* translators: 1: The WordPress error message. 2: The WordPress error code. */ __( 'Request failed due to an error: %1$s (%2$s)', 'wordpress-importer' ),
			...
			if ( 200 !== $remote_response_code ) {
				...
				return new WP_Error( 'import_file_error',
				                     sprintf( /* translators: 1: The HTTP error message. 2: The HTTP error code. */ __( 'Remote server returned the following unexpected result: %1$s (%2$s)',
				                                                                                                        'wordpress-importer' ),
